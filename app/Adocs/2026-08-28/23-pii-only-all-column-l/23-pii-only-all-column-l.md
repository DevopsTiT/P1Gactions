# PII Only All Column L

```
Need PII only — no HATS, no rawDataList, no total_logs?
  │
  ├─ UI: Logs and Events → Advanced mode
  │
  ├─ Run this query (PII keyword field names only)
  │     → hit_count per dt.host_group.id
  │
  ├─ hit_count > 0?
  │     → drill that app (seq 19 queries 19–21)
  │     → sample limit 10 only after count
  │
  └─ Later add HATS / Master columns if needed
```

| Question | Answer |
| --- | --- |
| What does this query do? | Counts log lines with **PII field names** only |
| What is excluded? | HATS, rawDataList, total_logs, Unicode regex |
| Scope | All **56** column L host groups |
| Result | `dt.host_group.id` and `hit_count` |

## Summary

This is the **simplest PII-only** query for all column L groups. It searches for insurance and HULFT **field name keywords** in log text (for example `insuredPerson`, `loginId`, `holderName`). No HATS columns, no Japanese Unicode regex, no extra counts. One row per app group, sorted by `hit_count` descending.
...
See also `23-pii-only-all-column-l-pii.dql`.
...
| Column | Meaning |
| --- | --- |
| `dt.host_group.id` | Column L app group |
| `hit_count` | Lines whose text contains a PII **field name** |

`hit_count` is a signal, not proof of a leak — sample a few lines only after you see non-zero counts.
...
```
56 column L host groups
  → filter insurance OR HULFT field name in content
  → count per app group
  → sort hit_count desc
```
...
| File | Role |
| --- | --- |
| `23-pii-only-all-column-l.md` | This answer |
| `23-pii-only-all-column-l-pii.dql` | Copy-paste query file |
